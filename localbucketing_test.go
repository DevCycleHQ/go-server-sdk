...
func TestDevCycleLocalBucketing_Initialize(t *testing.T) {
	...
	var err error
	err = localBucketing.Initialize(context.Background(), test_environmentKey, &DVCOptions{}, NewConfiguration(&DVCOptions{}))
	if err != nil {
		t.Fatal(err)
	}
}
...
func TestDevCycleLocalBucketing_GenerateBucketedConfigForUser(t *testing.T) {
	...
	err = localBucketing.StoreConfig(context.Background(), test_config)
	if err != nil {
		t.Fatal(err)
	}

	err = localBucketing.SetPlatformData(context.Background(), []byte(`{"platform": "golang-testing", "sdkType": "server", "platformVersion": "testing", "deviceModel": "testing", "sdkVersion":"testing"}`))
	if err != nil {
		t.Fatal(err)
	}

	genConfig, err := localBucketing.GenerateBucketedConfigForUser(context.Background(),
...
func BenchmarkDevCycleLocalBucketing_GenerateBucketedConfigForUser(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_, err := localBucketing.GenerateBucketedConfigForUser(context.Background(),
			[]byte(`{"user_id": "j_test", "platform": "golang-testing", "sdkType": "server", "platformVersion": "testing", "deviceModel": "testing", "sdkVersion":"testing"}`))
		if err != nil {
			b.Fatal(err)
		}
